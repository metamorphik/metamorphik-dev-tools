...
// NOTE: ensure enableHorizonAttributes() is called once at startup (e.g. in index.tsx).
import { enableHorizonAttributes } from '../../src/magik-attribute'; 
enableHorizonAttributes();

import {
  useEmitToEventHorizon,
  useEmitToNamedEventHorizon,
  useOnEvent,
  useOnNamedEventHorizon,
} from '../../src/react-horizon';

// Optional: helps TS accept magic props on custom components (they're stripped at runtime)
type HorizonMagicProps = {
...
function Module({ name }: { name: string } & HorizonMagicProps) {
  const emitLocal = useEmitToEventHorizon();
  const emitGlobal = useEmitToNamedEventHorizon('global');

  // ✅ call both hooks; choose at runtime (no conditional hooks)
  const emitProject = name.startsWith('A') ? useEmitToNamedEventHorizon('project:A') : useEmitToNamedEventHorizon('project:B');

  const [count, setCount] = React.useState(0);
  const [flagsView, raise] = useFlagAnimation();

  // Local scope reactions
  useOnEvent('flag:red', () => { setCount(c => c + 1); raise(); });
  // Also react to global
  ...
  return (
    <div className="moduleCircle">
      ...
      <div style={{ display:'flex', gap:6, flexWrap:'wrap', justifyContent:'center' }}>
        <button className="chip" onClick={() => emitLocal('flag:red', { from: name })}>
          Raise a red flag
        </button>
...
function ProjectCircle({ name, children }: { name: string; children: React.ReactNode } & HorizonMagicProps) {
  const [count, setCount] = React.useState(0);
  const [flagsView, raise] = useFlagAnimation();
  ...
  return (
    <div className="projectCircle">
      ...
      <div className="projectTitle">
        Project {name} <span className="bubble">{count}</span>
      </div>
      <div className="modulesWrap">{children}</div>
      <div className="controls">
        <button className="btn btn--warn" onClick={() => emitProject('flag:red', { scope: name })}>
          Raise flag in Project {name}
        </button>
      </div>
    </div>
  );
...
function GlobalShell({ children }: { children: React.ReactNode } & HorizonMagicProps) {
  const [count, setCount] = React.useState(0);
  const [flagsView, raise] = useFlagAnimation();
    ...
    <div className="shell">
      ...
      <div className="globalCircle">
        ...
        <div className="globalHead">
          <div className="stat">
            Global flags <span className="bubble">{count}</span>
          </div>
          <button className="btn btn--danger" onClick={() => emitGlobal('flag:red', { scope: 'global' })}>
            Raise a global red flag
          </button>
        </div>
        {children}
      </div>
...
export default function App() {
  return (
    // 🔮 MAGIC HAPPENS HERE (attributes on component *calls*, so hooks are in-scope)
    <GlobalShell definesHorizon="global" horizonLog>
      <div className="projectsRow">
        <ProjectCircle name="A" definesHorizon="project:A">
          <Module name="A-1" definesHorizon="module:A1" />
          <Module name="A-2" definesHorizon="module:A2" />
        </ProjectCircle>

        <ProjectCircle name="B" definesHorizon="project:B">
